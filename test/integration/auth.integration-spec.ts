...
describe('Auth module — registration against a real Postgres', () => {
  ...
  // Two calls, not three (see docs/adr/0006, issue #11) — the new password
  // takes effect immediately on the second call, no pending/confirm step,
  // since there's no new value that needs delivering and confirming first.
  describe('change password', () => {
    ...
    it('completes the full happy path: step-up then change swaps the live password', async () => {
      ...
      const { tokens } = await loginAndVerify(user.email, CURRENT_PASSWORD);
      const { challengeId, code } = await initiateStepUp(tokens.accessToken);
    ...
    it('rejects the wrong current password, leaving the credential unchanged', async () => {
      ...
      const unchanged = await credentialRepo.findOneByOrFail({
        userId: user.id,
      });
      expect(
        await bcrypt.compare(CURRENT_PASSWORD, unchanged.passwordHash),
      ).toBe(true);
    });
    ...
    it("rejects change-password using another user's step-up challenge", async () => {
      ...
      const unchangedB = await credentialRepo.findOneByOrFail({
        userId: userB.id,
      });
      expect(
    ...
    it('revokeOtherSessions: true revokes every other session but leaves the confirming one active', async () => {
      ...
      await loginAndVerify(user.email, CURRENT_PASSWORD);
      const { tokens: secondLoginTokens } = await loginAndVerify(
        user.email,
        CURRENT_PASSWORD,
      );
