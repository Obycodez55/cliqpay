...
- **Minimal comments — the default for any given method is zero.** Comments only where the *why* genuinely isn't obvious from the code (a workaround, a non-obvious invariant, a subtle race the implementation guards against) — never comments that restate what the code does. Don't cite an issue number, ADR, or doc section as a stand-in for the reasoning itself — either the *why* is worth a short sentence inline, or it doesn't need a comment at all. A reader shouldn't have to go open something else to find out why a line exists. If most methods in a file end up with a comment, that's the bar set too low, not unusually subtle code — most methods should have none. When reviewing a diff (own work or a spawned session's), cut any comment that doesn't survive this test before approving.
...
- **Repository access: `@InjectRepository` when a service never needs a transaction-scoped repo; `@InjectDataSource` + `dataSource.getRepository(...)` when it does.** These aren't interchangeable style preferences — a service that runs multi-entity atomic writes (`runInTransaction`, `manager.getRepository(...)` inside a transaction — see `ledger.service.ts` and the `auth` services that do this) needs the `DataSource` regardless, so adding `@InjectRepository` on top would mean two injection styles doing the same job in one class. A service that only ever does single-entity `save`/`find`/`delete` (e.g. `notifications`) has no transactional need pulling it toward `DataSource`, and `@InjectRepository` reads better there — the constructor itself tells you which tables the service touches. Pick per service based on whether it transacts, not by matching whatever the rest of the codebase happens to do.
